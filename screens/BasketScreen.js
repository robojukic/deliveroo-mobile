...
const BasketScreen = () => {
    ...
    useMemo(() => {
        const groupedItems = items.reduce((results, item) => {
          ...
          return results;
        }, {}); // Provide an empty object as initial value
      
        setGroupedItemsInBasket(groupedItems);
      }, [items]);

  return (
    <SafeAreaView className="pt-11 flex-1 bg-white">
      <View className="flex-1 bg-gray-100">
        ...
        <View className="flex-row items-center space-x-4 px-4 py-3 bg-white my-5">
            <Image source={{uri:'https://cdn.britannica.com/36/123536-050-95CB0C6E/Variety-fruits-vegetables.jpg'}}
        ...
        <ScrollView className="divide-y divide-gray-200">
            {Object.entries(groupedItemsInBasket).map(([key,items])=>(
                <View key={key} className="flex-row items-center space-x-3 bg-white py-2 px-5">
                    ...
                    <TouchableOpacity>
                        <Text className="text-[#00CCBB]" onPress={()=>dispatch(removeFromBasket({id:key}))}>Remove</Text>
                    </TouchableOpacity>
                </View>
            ))}
        </ScrollView>
        <View className="p-5 bg-white mt-5 space-y-4">
            <View className="flex-row justify-between">
                ...
                <Text className="text-gray-400">{formattedBasketTotal} €</Text>
            </View>
            <View className="flex-row justify-between">
